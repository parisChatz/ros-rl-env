...
def stabilize_camera_with_operator_input_fixed(
    q_drone,
    target_pan_op=0,
    target_tilt_op=0,
    target_roll_op=0,
    global_target=Quaternion(1, 0, 0, 0),
):
    """
...
class PTZCameraController:
    ...
    def publish_ptz_vals_callback(self, event=None):
        ...
        if self.drone_quat is not None:
            ...
            # pan, tilt, roll = stabilize_camera_with_operator_input_fixed(
            #     self.drone_quat,
            #     self.pan_angle_req,
            #     self.tilt_angle_req,
            #     target_roll_op=self.roll_angle_req,
            # )

            pan, tilt, roll = stabilize_camera_with_roll(
                self.drone_quat,
            )
