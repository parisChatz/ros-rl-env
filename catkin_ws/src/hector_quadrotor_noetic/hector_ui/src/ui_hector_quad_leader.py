...
# Subscribers
posicionLider_sub = rospy.Subscriber(
    "/uav1/ground_truth/state", Odometry, pose_callback
)
orientaLider_sub = rospy.Subscriber(
    "/uav1/ground_truth_to_tf/pose", PoseStamped, rot_callback
)
...
# ---------------------------- Botones de Control -------------------------------
ttk.Button(mainframe, text="Turn Left", command=ccw_fun).grid(
    column=1, row=5, sticky=tk.W
)
ttk.Button(mainframe, text="Fordward", command=forward_fun).grid(
    column=2, row=5, sticky=tk.W
)
ttk.Button(mainframe, text="Turn Right", command=cw_fun).grid(
    column=3, row=5, sticky=tk.W
)


ttk.Button(mainframe, text="Left", command=left_fun).grid(column=1, row=6, sticky=tk.W)
ttk.Button(mainframe, text="Hover", command=hover_pub).grid(
    column=2, row=6, sticky=tk.W
)
ttk.Button(mainframe, text="Right", command=right_fun).grid(
    column=3, row=6, sticky=tk.W
)

ttk.Button(mainframe, text="Up", command=up_fun).grid(column=1, row=7, sticky=tk.W)
ttk.Button(mainframe, text="Backward", command=backward_fun).grid(
...
ttk.Button(mainframe, text="Take Off", command=takeoff_fun).grid(
    column=3, row=4, sticky=tk.W
)
